...
This project provides a command-line tool to convert EPUB ebooks into audiobooks. It supports multiple Text-to-Speech providers including [Microsoft Azure Text-to-Speech API](https://learn.microsoft.com/en-us/azure/cognitive-services/speech-service/rest-text-to-speech), [EdgeTTS](https://github.com/rany2/edge-tts) (free, no API key required), [OpenAI Text-to-Speech API](https://platform.openai.com/docs/guides/text-to-speech), [Piper TTS](https://github.com/rhasspy/piper) (local), and [xAI TTS](https://api.x.ai/v1/tts) to generate the audio for each chapter in the ebook. The output audio files are optimized for use with [Audiobookshelf](https://github.com/advplyr/audiobookshelf).

<!-- *This project was developed with the help of ChatGPT.* -->

## Recent Updates

- 2026-04-21: Added xAI Text-to-Speech provider support
- 2026-04-21: Added Apple Books (.m4b) export to WebUI
- 2026-04-21: Default output folder now uses `<title>_-_<author>` format when not specified
- 2026-04-21: Updated dependencies (Gradio 6.7.0, requests 2.33.0) for security fixes
...
- For using *xAI TTS*, xAI [API Key](https://x.ai/api) is required.
...
export XAI_API_KEY=<your_xai_api_key>          # For xAI TTS
```
...
- **TTS Provider Selection**: Easy switching between Azure, OpenAI, Edge, Piper, and xAI TTS with provider-specific options
...
- **Apple Books Export**: Merge chapters into a single .m4b file with chapter markers for Apple Books compatibility

### Using the WebUI

1. **Upload your EPUB file** using the file selector
2. **Choose your TTS provider** from the tabs (OpenAI, Azure, Edge, Piper, or xAI)
3. **Configure provider-specific settings**:
   ...
   - **xAI**: Choose voice, language, sample rate, and bit rate
...
positional arguments:
  ...
  output_folder         Path to the output folder (optional, defaults to audiobook_output/<title>_-_<author>)

options:
  -h, --help            show this help message and exit
  --tts {azure,openai,edge,piper,xai}
                        Choose TTS provider (default: azure). azure: Azure
                        Cognitive Services, openai: OpenAI TTS API, edge: Edge
                        TTS (free), piper: Piper TTS (local), xai: xAI TTS API.
                        When using azure, environment variables MS_TTS_KEY and
                        MS_TTS_REGION must be set. When using openai,
                        environment variable OPENAI_API_KEY must be set. When
                        using xai, environment variable XAI_API_KEY must be set.
...
xai specific:
  --xai_sample_rate XAI_SAMPLE_RATE
                        Sample rate for xAI TTS output (default: 22050)
  --xai_bit_rate XAI_BIT_RATE
                        Bit rate for xAI TTS output (default: 128)
```  
...
```bash
docker run -i -t --rm -v ./:/app -e XAI_API_KEY=$XAI_API_KEY ghcr.io/p0n1/epub_to_audiobook your_book.epub audiobook_output --tts xai
```
...
Check https://x.ai/api for API access and pricing details.
...
1. **Basic conversion using Edge with default settings**  
   ...
   ```
### Examples Using xAI TTS

1. **Basic conversion using xAI with default settings**  
   This command will convert an EPUB file to an audiobook using xAI's default TTS settings.

   ```sh
   python3 main.py "path/to/book.epub" "path/to/output/folder" --tts xai
   ```

2. **xAI conversion with custom voice and sample rate**  
   Converts an EPUB file to an audiobook using a specific xAI voice and sample rate.

   ```sh
   python3 main.py "path/to/book.epub" "path/to/output/folder" --tts xai --voice_name "rex" --xai_sample_rate 44100
   ```
